...
--
DROP TRIGGER IF EXISTS `activity_logs_insert`;
DELIMITER $$
CREATE TRIGGER `activity_logs_insert` AFTER INSERT ON `activity_logs` FOR EACH ROW BEGIN
INSERT INTO activity_logs_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `activity_logs_sync`
--

DROP TABLE IF EXISTS `activity_logs_sync`;
CREATE TABLE IF NOT EXISTS `activity_logs_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `port` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` datetime NOT NULL,
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '0=pending, 1=sync success, 2=processing,3=error',
...
-- Triggers `checklists`
--
DROP TRIGGER IF EXISTS `checklists_insert`;
DELIMITER $$
CREATE TRIGGER `checklists_insert` AFTER INSERT ON `checklists` FOR EACH ROW BEGIN
INSERT INTO checklists_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `checklists_update`;
DELIMITER $$
CREATE TRIGGER `checklists_update` AFTER UPDATE ON `checklists` FOR EACH ROW BEGIN
DELETE FROM checklists_sync where id=old.id;
INSERT INTO checklists_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `checklists_sync`
--

DROP TABLE IF EXISTS `checklists_sync`;
CREATE TABLE IF NOT EXISTS `checklists_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `port` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` datetime NOT NULL,
...
--
-- Triggers `deleted_visas`
--
DROP TRIGGER IF EXISTS `deleted_visas_insert`;
DELIMITER $$
CREATE TRIGGER `deleted_visas_insert` AFTER INSERT ON `deleted_visas` FOR EACH ROW BEGIN
INSERT INTO deleted_visas_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `deleted_visas_update`;
DELIMITER $$
CREATE TRIGGER `deleted_visas_update` AFTER UPDATE ON `deleted_visas` FOR EACH ROW BEGIN
DELETE FROM deleted_visas_sync where id=old.id;
INSERT INTO deleted_visas_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `deleted_visas_sync`
--

DROP TABLE IF EXISTS `deleted_visas_sync`;
CREATE TABLE IF NOT EXISTS `deleted_visas_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `port` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` datetime NOT NULL,
...
(0xddd07e0a49185ece8d49fd28c399d83a, NULL, 'PHN', 'F9615C43-E117-49F9-BA54-5F3FEA806266', 'PC 1', 'Windows 11 Pro', '22H2', 'app', '1.0.0', 'PostmanRuntime/7.34.0', '192.168.88.209', '2023-10-31 10:35:56', '2023-10-31 10:35:56', '2023-10-26 10:11:58', 1, NULL);
...
-- Triggers `passports`
--
DROP TRIGGER IF EXISTS `passports_insert`;
DELIMITER $$
CREATE TRIGGER `passports_insert` AFTER INSERT ON `passports` FOR EACH ROW BEGIN
INSERT INTO passports_sync(id, created_at) VALUES(new.pid, now());
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `passports_update`;
DELIMITER $$
CREATE TRIGGER `passports_update` AFTER UPDATE ON `passports` FOR EACH ROW BEGIN
DELETE FROM passports_sync where id=old.pid;
INSERT INTO passports_sync(id, created_at) VALUES(new.pid, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `passports_sync`
--

DROP TABLE IF EXISTS `passports_sync`;
CREATE TABLE IF NOT EXISTS `passports_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `port` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` datetime NOT NULL,
...
--
-- Triggers `printed_visas`
--
DROP TRIGGER IF EXISTS `printed_visas_insert`;
DELIMITER $$
CREATE TRIGGER `printed_visas_insert` AFTER INSERT ON `printed_visas` FOR EACH ROW BEGIN
INSERT INTO printed_visas_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `printed_visas_update`;
DELIMITER $$
CREATE TRIGGER `printed_visas_update` AFTER UPDATE ON `printed_visas` FOR EACH ROW BEGIN
DELETE FROM printed_visas_sync where id=old.id;
INSERT INTO printed_visas_sync(id, created_at) VALUES(new.id, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `printed_visas_sync`
--

DROP TABLE IF EXISTS `printed_visas_sync`;
CREATE TABLE IF NOT EXISTS `printed_visas_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `created_at` datetime NOT NULL,
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '0=pending, 1=sync success, 2=processing,3=error',
...
(0x0b096f4447765a12815e9edbf8008c3f, 'PHN', 0x02f5d68f76607b5bcda97825958fb56b, 'MDJmNWQ2OGYtNzY2MC03YjViLWNkYTktNzgyNTk1OGZiNTZi2sWKgfsORPCZ2vKKIsEy86chWUvePKFRTEISJE0uozMZSXuhQae9LzDFcBvi', '2023-10-26 16:40:33', '2024-10-26 16:40:33', NULL, '192.168.88.209', 'F9615C43-E117-49F9-BA54-5F3FEA806266', 'PostmanRuntime/7.34.0', '2023-10-26 16:40:33', '2023-10-26 16:40:33'),
(0x95e99653cb2b5c68a4002be6c024bfdb, 'PHN', 0xcdba11b0e0de58f4a4d5eb528ee9f408, 'Y2RiYTExYjAtZTBkZS01OGY0LWE0ZDUtZWI1MjhlZTlmNDA49RHrJHvVttxXOY3nojCMBKpzHdWTPovP9dCafEqztuqXbPGnqxTDyvIclRm4', '2023-10-27 11:13:36', '2024-10-27 11:13:36', NULL, '192.168.88.209', 'F9615C43-E117-49F9-BA54-5F3FEA806266', 'PostmanRuntime/7.34.0', '2023-10-27 11:13:36', '2023-10-27 11:13:36'),
(0xa94ed457f1fe5a1eaf4ea366de6fee8e, 'PHN', 0x02f5d68f76607b5bcda97825958fb56b, 'MDJmNWQ2OGYtNzY2MC03YjViLWNkYTktNzgyNTk1OGZiNTZib2b0YnkWJ2z2iniVRusKLnS2L6hGgrjiDJALz7a1kmTqoHwxbAfhO9BESHas', '2023-10-26 10:19:18', '2024-10-26 10:19:18', NULL, '192.168.88.209', NULL, 'PostmanRuntime/7.34.0', '2023-10-26 10:19:18', '2023-10-26 10:19:18'),
(0xfbb7cc0d1cde5ab1b958b9c40c126f1c, 'PHN', 0x02f5d68f76607b5bcda97825958fb56b, 'MDJmNWQ2OGYtNzY2MC03YjViLWNkYTktNzgyNTk1OGZiNTZibzieJuFCeyu559KevCz75CoQwqDPETqVZMp3Pb8hb64VwnJeEB5CQsS9HnOo', '2023-10-26 15:16:22', '2024-10-26 15:16:22', '2023-10-26 16:57:40', '192.168.88.209', 'F9615C43-E117-49F9-BA54-5F3FEA806266', 'PostmanRuntime/7.34.0', '2023-10-26 15:16:22', '2023-10-26 15:16:22');
...
-- Triggers `visas`
--
DROP TRIGGER IF EXISTS `visas_insert`;
DELIMITER $$
CREATE TRIGGER `visas_insert` AFTER INSERT ON `visas` FOR EACH ROW BEGIN
INSERT INTO visas_sync(id, created_at) VALUES(new.vid, now());
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `visas_update`;
DELIMITER $$
CREATE TRIGGER `visas_update` AFTER UPDATE ON `visas` FOR EACH ROW BEGIN
DELETE FROM visas_sync WHERE id=old.vid;
INSERT INTO visas_sync(id, created_at) VALUES(new.vid, now());
END
$$
DELIMITER ;
...
--
-- Table structure for table `visas_sync`
--

DROP TABLE IF EXISTS `visas_sync`;
CREATE TABLE IF NOT EXISTS `visas_sync` (
  `sid` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `id` binary(16) NOT NULL,
  `created_at` datetime NOT NULL,
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '0=pending, 1=sync success, 2=processing,3=error',
